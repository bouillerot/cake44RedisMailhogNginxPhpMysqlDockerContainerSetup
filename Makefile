...
UID             := $(shell id -u)
UNAME           := $(shell uname -s)
PHP_DEV_PATH	:= ".docker/php/dev"
PATH_TO_FILE    := "../app/composer.json"
...
init: do.copy
	...
	@docker-compose build --build-arg UID=$(UID) --build-arg ENV=dev --build-arg HOST_OS=$(shell uname -s)
	@$(DC_UP)
init.nocache: do.copy
	@printf $(DOCKER_ICO) && printf "$(GOOD)running docker build --no-cache and up $(E)"
	@mkdir -p app && touch app/.gitkeep
	@docker-compose build --build-arg UID=$(UID) --build-arg ENV=dev --no-cache --build-arg HOST_OS=$(shell uname -s)
	@$(DC_UP)
...
#
do.copy:
	@cp $(PHP_DEV_PATH)/conf.d/20-overrides.ini.dev $(PHP_DEV_PATH)/conf.d/20-overrides.ini
